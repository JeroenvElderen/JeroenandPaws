const { supabaseAdmin, ensureClientProfile, hashPassword } = require('./_lib/supabase');
...
module.exports = async (req, res) => {
  if (req.method === 'GET') {
    ...
    if (!clientResult.data.hashed_password) {
      res.statusCode = 403;
      res.end(JSON.stringify({ message: 'Please set your password to access your profile.' }));
      return;
    }
  ...
  if (req.method === 'PUT') {
    ...
    res.setHeader('Content-Type', 'application/json');
    res.end(JSON.stringify({ message: 'Password updated', client: updateResult.data }));
    return;
  }
