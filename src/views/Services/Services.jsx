        ...
        <div className="container">
          <div className="header is-align-center">
            ...
            <p className="subheading max-width_large">
              Choose from trusted walks, engaging adventures, supportive training, and cozy stays. Every option is delivered with the same friendly, professional care.
            </p>
          </div>

          <div className="w-layout-grid grid_3-col tablet-1-col gap-small">
            {services.map((service) => (
              <div key={service.title} className="card height_100percent">
                <Link href={service.path} className="card-link flex_vertical gap-small height_100percent w-inline-block">
                  <div className="image-ratio_4x3">
                    <img src={service.imageSrc} alt={service.imageAlt} className="image_cover" loading="lazy" />
                  </div>
                  <div className="card_body flex_vertical gap-xsmall flex_child-expand">
                    <div className="eyebrow">Service</div>
                    <div className="heading_h4 margin-bottom_none">{service.title}</div>
                    <p className="paragraph_small text-color_secondary margin-bottom_none">{service.description}</p>
                    <div className="text-button">
                      <div>Explore {service.title.toLowerCase()}</div>
