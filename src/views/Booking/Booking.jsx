...
const serviceGroups = [
  ...
  },
  {
    ...
    description:
      "Boarding with bedtime routines, morning walks, and check-ins.",
    variants: [
      {
        id: "overnight-stay",
...
const Booking = () => {
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(
        "jp_calendar_bookings",
        JSON.stringify(calendarBookings)
      );
    } catch (error) {
  ...
  const updateServiceQuery = useCallback(
    (serviceId) => {
      ...
      router.replace({ pathname: "/booking", query: nextQuery }, undefined, {
        shallow: true,
      });
    },
  ...
  const getCalendarDetails = (bookingContext = null) => {
    ...
    return {
      title: `${serviceInfo.title} — ${
        bookingContext?.dogName || dogName || "Dog booking"
      }`,
      start,
      end,
  ...
  const buildGoogleCalendarLink = (details) => {
    ...
    return `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(
      details.title
    )}&dates=${formatDate(details.start)}/${formatDate(
      details.end
  ...
  const handleSubmit = async (event) => {
    ...
    if (
      ...
      new Date(endDate) < new Date(selectedSlot.start)
    ) {
    ...
    try {
      ...
      setTimeout(() => {
        const confirmationCode = `${selectedService.id}-${new Date(
          selectedSlot.start
        ).getTime()}`;
        setSuccessMessage(
          `You are booked for ${selectedService.title} on ${new Date(
            selectedSlot.start
          ).toLocaleString()} · Confirmation: ${confirmationCode}`
        );
        const newBooking = {
          id: confirmationCode,
          service: selectedService,
          slot: selectedSlot,
          endDate: selectedService.multiDay
            ? endDate
  ...
  const formatBookingDateRange = (booking) => {
    ...
    return booking.service.multiDay
  ...
  return (
    <main className="booking-shell">
      <div className="booking-shell__content">
        <section className="booking-hero">
          <div>
            <p className="eyebrow">Book instantly</p>
            <h1 className="booking-hero__title">
              Signature Jeroen &amp; Paws calendar
            </h1>
            <p className="booking-hero__subtitle">
              logins, no Cal.com, and no third-party scheduler—everything saves
              locally and exports to your own calendar.
            </p>
            ...
            {isServiceLocked && (
              <div className="service-lock-note">
                You opened a service-specific booking link. Service selection is
                locked to {selectedService.title}. Want to switch?{" "}
                <Link href="/booking" className="service-lock-note__link">
                  Start from the booking hub.
                </Link>
              </div>
            )}
            {error && (
              <div className="booking-banner booking-banner--error">
                {error}
              </div>
            )}
            {successMessage && (
              <div className="booking-banner booking-banner--success">
                {successMessage}
              </div>
            )}
          </div>
        ...
        <section className="service-switcher">
          <div className="service-switcher__header">
            <div>
              ...
              <p className="muted">
                Attach the matching link to your homepage buttons for a
                  ...
                  {group.variants.map((variant) => {
                    ...
                    return (
                      <button
                        key={variant.id}
                        type="button"
                        className={`variant-tile variant-tile--${
                          variant.accent
                        }${isActive ? " is-active" : ""}${
                      ...
                      >
                        <div className="variant-tile__top">
                          ...
                          {isActive && (
                            <span
                              className="legend__dot legend__dot--active"
                              aria-hidden="true"
                            />
                          )}
                        </div>
            ...
            {selectedService.multiDay && (
              <div className="sidebar-card">
                ...
                <p className="muted small">
                  Blocks your calendar until checkout.
                </p>
              </div>
            )}
          </aside>
            ...
            <div className="times-card">
              <div className="times-card__header">
                ...
                {selectedService.multiDay && (
                  <div className="pill pill--ghost">
                    Multi-day stay · checkout {endDate}
                  </div>
                )}
              </div>
              <div className="times-card__slots">
                ...
                {!isLoading && availability.length === 0 && (
                  <div className="muted">No open times on this day.</div>
                )}
                {!isLoading &&
                  availability.map((slot) => (
                    <button
                      ...
                      type="button"
                      className={`slot-chip${
                        selectedSlot?.start === slot.start ? " is-selected" : ""
                      }`}
        ...
        <section className="booking-card booking-card--details">
          <div className="booking-card__header">
            ...
            <p className="muted">
              We’ll email you to confirm and share pickup notes.
            </p>
          </div>
          <form className="booking-form" onSubmit={handleSubmit}>
            ...
            <div className="booking-form__actions">
              ...
              <div className="tiny-note">
                Built in-house to match your calendar style
              </div>
            </div>
          </form>
        </section>
        ...
        <section className="booking-card booking-card--calendar">
          <div className="booking-card__header">
            ...
            <p className="muted">
              Saved bookings stay in your browser and export to your own
              calendar—no external scheduling platforms.
            </p>
          </div>
          {calendarBookings.length === 0 && (
            <div className="empty-note">
              Confirm a booking to populate your private list.
            </div>
          )}
          {calendarBookings.length > 0 && (
                ...
                <div key={booking.id} className="calendar-entry">
                  <div className="calendar-entry__info">
                    <div className="calendar-entry__header">
                      <span
                        className={`pill variant-pill variant-pill--${booking.service.accent}`}
                      >
                    ...
                    <p className="calendar-entry__meta muted small">
                      {booking.clientName || "Client pending"}
                    </p>
        ...
        {lastBooking && (
          <section className="calendar-card add-on">
            <div>
              ...
              <p className="muted">
                Drop this booking into Google Calendar or download an .ics file
                for Outlook/Apple Calendar. We fill in the details so you never
                lose track.
              </p>
            </div>
            {(() => {
              ...
              return (
                <div className="calendar-actions">
                  ...
                  <a
                    className="submit-button ghost"
                    href={icsHref}
                    download={`${selectedService.id}-booking.ics`}
                  >
                    Download .ics
                  </a>
